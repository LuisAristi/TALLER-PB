...
void reporte_total_horas_trabajadas(int *codigo, int *sueldo_semanal, int *horas_trabajadas, int n);
void reporte_total_pagado_empresa(int *codigo, int *sueldo_semanal, int n);
    ...
    inicio:
        ...
        case 1:
            while (opc == 1)
            {
                ...
                cout << endl
                     << endl
                     << "------ presione cualquier numero distinto de 1 para salir o 1 para repetir: ";
                cin >> opc;
            }
        ...
        case 2:
            ...
            label1: // goto es literalmente ir a, cuando decimos goto label1; regresamos aca, si usamos break en vez de goto sale error en el compilador.
                ...
                {
                    cout << "recuerde ingresar un numero primo" << endl;
                    goto label1; // el goto es muchisimo mejor que implementar ciclos
                }
                ...
                if (malvada(binarios[0], size) % 2 == 0)
                { //(no importa) lo importante poder contar cuantos 1 hay y por consiguiente saber si el numero es malvado
                    cout << "es un numero malvado !!!" << endl;
                }
                else
                    cout << "no es un numero malvado :()" << endl;

                cout << endl
                     << endl
                     << "------ presione cualquier numero distinto de 2 para salir o 2 para repetir: "; // no se me ocurre otro metodo mejor (los hay)
                cin >> opc;
            }
            goto inicio;
        ...
        case 3:
            ...
            int n;
            cout << "ingrese la cantidad de choferes: ";
            cin >> n;
            ...
            menu3:
                ...
                case 3:
                    ...
                    goto menu3;
                case 4:
                    reporte_total_horas_trabajadas(codigo, sueldo_semanal, horas_trabajadas, n);
                    reporte_total_pagado_empresa(codigo, sueldo_semanal, n);
                    goto menu3;
                case 5:
                    goto inicio;
                default:
...
void consulta(int *codigo, int *sueldo_hora, int *horas_trabajadas, int *sueldo_semanal, int n)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (codigo[i] == consulta)
        {
            ...
            cout << "Sueldo por hora: " << sueldo_hora[i] << " dolares por hora" << endl;
            cout << "Sueldo semanal: " << sueldo_semanal[i] << " dolares" << endl;
...
void reporte_total_horas_trabajadas(int *codigo, int *sueldo_semanal, int *horas_trabajadas, int n)
{
    // Calcular el total de horas promedio
    int total_horas_promedio = 0;

    for (int i = 0; i < n; ++i)
    {
        total_horas_promedio += horas_trabajadas[i];
    }

    // Calcular el porcentaje de choferes que trabajaron menos de las horas promedio
    int choferes_menos_horas_promedio = 0;

    for (int i = 0; i < n; ++i)
    {
        if (horas_trabajadas[i] < total_horas_promedio / n)
        {
            choferes_menos_horas_promedio++;
        }
    }
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (horas_trabajadas[j] < horas_trabajadas[j + 1])
            {
                // Intercambiar valores usando variables temporales
                int temp1 = sueldo_semanal[j];
                sueldo_semanal[j] = sueldo_semanal[j + 1];
                sueldo_semanal[j + 1] = temp1;

                int temp2 = codigo[j];
                codigo[j] = codigo[j + 1];
                codigo[j + 1] = temp2;

                int temp3 = horas_trabajadas[j];
                horas_trabajadas[j] = horas_trabajadas[j + 1];
                horas_trabajadas[j + 1] = temp3;
            }
        }
    }
    ...
    for (int i; i < n; i++)
    {
        cout << "Codigo del Chofer: " << codigo[i] << endl;
        cout << "Total horas: " << horas_trabajadas[i] << " horas" << endl;
        cout << "Total cancelado " << sueldo_semanal[i] << " dolares" << endl;
    }

    cout << "Porcentaje de choferes con menos horas que el promedio: " << (choferes_menos_horas_promedio * 100.0) / n << "%" << endl
         << endl;
}
...
void reporte_total_pagado_empresa(int *codigo, int *sueldo_semanal, int n)
{
    ...
    for (int i = 0; i < n - 1; ++i)
    {
        for (int j = 0; j < n - i - 1; ++j)
        {
            if (sueldo_semanal[j] < sueldo_semanal[j + 1])
            {
                // Intercambiar valores usando variables temporales
                int temp1 = sueldo_semanal[j];
                sueldo_semanal[j] = sueldo_semanal[j + 1];
    ...
    cout << "Total pagado por la empresa:" << endl;

    for (int i = 0; i < n; ++i)
    {
        cout << "Código del chofer: " << codigo[i] << endl;
        cout << "Total cancelado: " << sueldo_semanal[i] << " dolares" << endl;
        cout << "Total que pagará la empresa: " << total_pagado << " dolares" << endl
             << endl;
    }
}
